...
namespace ics {
  ...
  public:
    ...
    static bool my_gt(const std::string& a, const std::string& b) {
    return  a < b;
    }
...
T HashGraph<T>::edge_value(NodeName origin, NodeName destination) const {
      if (!has_edge(origin,destination)) {
        throw GraphError("Edge with origin: " + origin + "and destination: " + destination + " not in graph.");
      }
...
void HashGraph<T>::add_edge (NodeName origin, NodeName destination, T value) {
      add_node(origin); add_node(destination);
...
void HashGraph<T>::remove_node (NodeName node_name){
      ...
      for (auto &kv : node_values) {
        NodeSet node1 = kv.second.in_nodes;
        NodeSet node2 = kv.second.out_nodes;
        EdgeSet node3 = kv.second.in_edges;
        EdgeSet node4 = kv.second.out_edges;

        for (auto &k : node1)
          if (k == node_name)
            node_values[kv.first].in_nodes.erase(k);

        for (auto &k : node2)
          if (k == node_name)
            node_values[kv.first].out_nodes.erase(k);

        for (auto &k : node3)
         if (k.first == node_name || k.second == node_name) //k.second
          remove_edge(k.first, k.second);


        for (auto &k : node4)
         if (k.second == node_name || k.first == node_name) //k.first
          remove_edge(k.first, k.second);
...
void HashGraph<T>::load (std::ifstream& in_file, std::string separator) {
      ...
      while(getline(in_file,line)) {
        if (line.std::string::find(separator) == std::string::npos) {
            add_node(line);
          continue;
        }
        std::vector<std::string> hold = ics::split(line,separator);

        std::istringstream s(hold[2]);
        T val;
        s >> val;
          add_edge(hold[0],hold[1],val);
      }
     in_file.close();
}
