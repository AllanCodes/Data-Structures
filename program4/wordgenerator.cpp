...
//  read_corpus time                   = 0.5
//  print_corpus (sort not print) time = 0.093

//Array Implementations for Queue; HeapPriorityQueue, HashMap, and HashSet
//  read_corpus time                   = 0.766
//  print_corpus (sort not print) time = 0.234

//Briefly explain (in a comment below) what is interesting
//about the times above (and explain why it occurs).
//
//Well its definitely interesting that without the Hash implementation of the set
//both the read corpus and the print corpus time are less than the time for the
//Map and Set implementations of Hash.  I think the overhead here really takes the constant for the complexity
//up. When we have a hashset, compared to an arrayset, when we increase the number of bins, we need to re hash
//as well as create new LN* nodes for each bin before re adding all values back in.