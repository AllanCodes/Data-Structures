...
: hash(thash != undefinedhash<T> ? thash : chash){
...
HashSet<T,thash>::HashSet(const HashSet<T,thash>& to_copy, double the_load_threshold, int (*chash)(const T& element))
 ...
 hash = (hash == hashfunc(undefinedhash<T>) ? to_copy.hash : hash);
 load_threshold = the_load_threshold;
 bins = to_copy.bins;

 if (hash == to_copy.hash) {
  used = to_copy.used;
  this->set = copy_hash_table(to_copy.set,bins);
  return;
 }
 ...
 //hash
 for (int i = 0; i < bins; i++ )
  for (LN* l = to_copy.set[i]; l->next != nullptr; l=l->next)
   insert(l->value);
...
int i = 0;
 if (k != nullptr) { //already there
  return 0;
 }

 ensure_load_threshold(used + 1); //OKAY I was changing the number of bins using ensure load AFTER I hash compressed
 i = hash_compress(element);
 set[i] = new LN(element,set[i]); //points to trailer (if first)
 ++used;
 ++mod_count;
 return 1;
}
...
int HashSet<T,thash>::retain_all(const Iterable& i) {
 int count = 0;
 HashSet<T,thash> hs(i);

 for (int i = 0; i < bins; i++ ) {
  for (LN* l = set[i]; l->next != nullptr;) {
   if (!hs.contains(l->value)) {
    LN* to_delete = l->next;
    *l = *to_delete;
    delete to_delete;
    ++count;
...
std::ostream& operator << (std::ostream& outs, const HashSet<T,thash>& s) {
 ...
 for (int i = 0; i < s.bins; i++) {
  for(auto l = s.set[i]; l->next != nullptr; l = l->next) {
   outs << l->value;
...
typename HashSet<T,thash>::LN* HashSet<T,thash>::copy_list (LN* l) const {
 ...
 else return new LN(l->value, copy_list(l->next));
}
...
void HashSet<T,thash>::ensure_load_threshold(int new_used) {
 ...
 int saveBins = bins;
 bins = 2 * saveBins;
 LN** old = copy_hash_table(set,saveBins);
...
//used = new_used;
 ...
 for (int i = 0; i < saveBins; i++ ){
  for (LN* l = old[i]; l->next != nullptr; l = l->next) {
   int i = hash_compress(l->value);
   set[i] = new LN(l->value,set[i]);
  }
 }
}
...
//Iterator class definitions
    ...
    void HashSet<T,thash>::Iterator::advance_cursors() {
     ...
     if (current.second == nullptr || current.second->next == nullptr){
      ...
      for (int i = index + 1; i < ref_set->bins; i++) {
    ...
    bool HashSet<T,thash>::Iterator::operator == (const HashSet<T,thash>::Iterator& rhs) const {
     ...
     return this->current.first == rhsASI->current.first && this->current.second == rhsASI->current.second;
    }
