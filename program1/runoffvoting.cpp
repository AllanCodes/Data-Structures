...
//
//bool entry_gt(const PreferencesEntry& a, const PreferencesEntry& b) {
...
//        CandidateQueue tmp; //temp queue
//        for (int i = 1; i < vec.size(); i++) {
//            tmp.enqueue(vec[i]);
//        }
//        voters[vec[0]].enqueue_all(tmp);
//        tmp.clear();
//    }
//
//    return voters;
...
//    PreferencesEntryPQ sortPQ(preferences,entry_gt);
//
...
//}
//
//bool entry_a(const TallyEntry& a, const TallyEntry& b) {
//    return a.first < b.first;
//}
//
//bool entry_n(const TallyEntry& a, const TallyEntry& b) {
...
//
//    TallyEntryPQ sortPQ(has_higher_priority);
//    sortPQ.enqueue_all(tally);
//
...
//CandidateTally evaluate_ballot(const Preferences& preferences, const CandidateSet& candidates) { //typedef ics::ArrayMap<std::string,CandidateQueue> Preferences;
//    CandidateTally tally; //typedef ics::ArrayMap<std::string,int>
//
//    for (auto &kv : preferences) {
...
////  Set is returned.
//CandidateSet remaining_candidates(const CandidateTally& tally) { //typedef ics::ArrayMap<std::string,int>
//
//    CandidateSet left;
//    int votes = std::numeric_limits<int>::max();
//
//    for (auto &kv : tally) {
//        if (kv.second < votes) {
//            votes = kv.second;
...
//    try {
//
//        CandidateSet candidates; //store all candidates
//        std::stringstream a, n;
//        CandidateTally ct;
//        std::ifstream file;
//        ics::safe_open(file, "Enter file name", "../input files/votepref1.txt");
//        Preferences pref = read_voter_preferences(file);
//        print_voter_preferences(pref);
//
//        for (auto &kv : pref) {
//                candidates.insert_all(kv.second);
//        }
//
//        for (int i = 1; candidates.size() > 1; i++,candidates = remaining_candidates(ct)) {
//
//            ct = evaluate_ballot(pref, candidates);
//            a << "Vote count on ballot #" << i << ": candidates (alphabetically ordered) with remaining candidates = " << candidates;
//            print_tally(a.str(),ct,entry_a);
//            n << "Vote count on ballot #" << i << ": candidates (numerically ordered) with remaining candidates = " << candidates;
//            print_tally(n.str(), ct, entry_n);
//
//            a.str(std::string());
//            n.str(std::string());
//        }
...
//        }
//        else std::cout << "Not any unique winner: election is a tie among all the candidates remaining on the last ballot" << std::endl;
//
//
//
//
